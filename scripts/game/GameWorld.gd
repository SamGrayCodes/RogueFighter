...
const RESULTS_SCENE: String = "res://scenes/ui/ResultsScreen.tscn"
...
func _ready() -> void:
	...
	if NetworkManager.is_networked():
		...
		_setup_networked()
	elif not GameState.players.is_empty():
		_load_selected_stage()
		_setup_configured_local()
	else:
		_current_stage = _find_stage()
		_setup_dev_fallback()
...
## Dev / direct-run: use the characters and stage baked into the scene, and let
## the RulesEditor drive the first round (Phase 1-3 behavior).
func _setup_dev_fallback() -> void:
	_collect_preplaced_characters()
	_rules_editor.rules_applied.connect(_on_rules_applied, CONNECT_ONE_SHOT)
	_rules_editor.show()

## Local play launched from the menus: build the chosen stage + roster and start
## immediately using the configured rules/mode.
func _setup_configured_local() -> void:
	...
	for pd: GameState.PlayerData in GameState.players:
		var character: CharacterBase = _build_character(
			GameState.get_character_scene(pd.character_id), pd.player_index, pd.peer_id)
		_players.add_child(character)
...
## Networked: the server spawns one character per roster entry through the
## spawner; clients receive them. No-upgrades keeps the loop authoritative
## without the (unbuilt) networked upgrade-selection flow.
func _setup_networked() -> void:
	...
	for pd: GameState.PlayerData in GameState.players:
		var character: CharacterBase = _spawner.spawn({
			"scene": GameState.get_character_scene(pd.character_id),
...
func _spawn_character(data: Dictionary) -> Node:
	return _build_character(data["scene"], int(data["index"]), int(data["peer"]))

func _build_character(scene_path: String, index: int, peer: int) -> CharacterBase:
	var scene: PackedScene = load(scene_path) as PackedScene
	var character: CharacterBase = scene.instantiate() as CharacterBase
	character.player_index = index
	character.input_authority = peer
	character.name = "Player_%d" % index
	return character
...
func _collect_preplaced_characters() -> void:
	for child: Node in _players.get_children():
		if child is CharacterBase:
...
func _load_selected_stage() -> void:
	var scene_path: String = GameState.get_stage_scene(GameState.selected_stage_id)
	var packed: PackedScene = load(scene_path) as PackedScene
	if packed == null:
		_current_stage = _find_stage()
		return
	var old_stage: StageBase = _find_stage()
	if old_stage != null:
		remove_child(old_stage)
		old_stage.queue_free()
	var stage: StageBase = packed.instantiate() as StageBase
	add_child(stage)
	move_child(stage, 0)
...
# --- Presentation (runs on every peer) ---------------------------------------

func _gather_players() -> Array[CharacterBase]:
...
func _update_hud() -> void:
	var characters: Array[CharacterBase] = _gather_players()
	if characters.size() != _hud_player_count:
		_hud_player_count = characters.size()
		_hud.setup(characters)
	if _is_sim_authority():
		_hud.set_round(GameState.current_round)
		_hud.set_timer(_round_timer if _is_round_active else GameState.active_rules.round_time_limit)
